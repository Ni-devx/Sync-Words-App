...
const Sidebar: React.FC<SidebarProps> = ({ onClose }) => {
  ...
  // ★★★ 追加: 設定リンククリック時にバッジを消し、localStorageを更新 ★★★
  const handleSettingsClick = () => {
    const latestVersion = updates.length > 0 ? updates[0].version : null;
    if (latestVersion) {
      localStorage.setItem('lastSeenUpdateVersion', latestVersion);
    }
            ...
            return (
              <li key={item.href}>
                <Link
                  ...
                  `}
                  // ★★★ 修正: 設定リンクの場合のみ特別処理 ★★★
                  onClick={isSettings ? handleSettingsClick : onClose}
                >
                  <item.icon className="w-5 h-5" />
                  {item.name}
                  {/* ★★★ 追加: 通知バッジの表示 ★★★ */}
                  {isSettings && showUpdateBadge && (
